      ...
      details: [
        {
          ...
          detail: [
            ...
            '정렬(최신순, 인기순, 조회순)과 태그 필터링에 따라 API 호출 시 동적으로 쿼리 문자열을 생성하여 새로운 옵션 추가 시 간편하게 대응할 수 있도록 구현했습니다.',
          ],
        },
        {
      ...
      details: [
        {
          ...
          detail: [
            'useRef 및 scrollIntoView()를 활용해 현재 페이지에 해당하는 미리보기 버튼을 중앙으로 스크롤하는 기능을 구현함으로써 페이지 전환 시 동기화된 미리보기를 제공했습니다.',
          ],
        },
        {
          contribution: 'Canvas를 활용한 줄거리 텍스트 이미지 변환',
          detail: [
            'HTML Canvas를 활용해 동적으로 텍스트를 이미지로 변환하고, measureText로 줄의 너비를 측정해 줄바꿈 처리를 하여 가독성을 향상시켰습니다.',
          ],
        },
        {
