...
export const project0: ProjectDetail = {
  ...
  techStack: [
    ...
    },
    {
      ...
      reason: [
        ...
        '상호작용이 많은 서비스이므로 변동성이 큰 스타일링을 효과적으로 처리하기 위해 Styled-components를 선택했습니다.',
      ],
    },
  ],
  contributions: {
    '아바타 커스터마이징 및 오늘의 코디 추천': {
      ...
      details: [
        {
          contribution: '아이템 카테고리화 및 관리',
          detail: [
            '아이템을 카테고리별로 분류하고 특정 폴더 내의 파일을 자동으로 가져오는 Webpack의 기능인 require.context를 통해 이미지를 효율적으로 로드해 코드의 가독성과 유지보수성을 향상시켰습니다.',
          ],
        },
        {
        ...
        {
          contribution: '실시간 날씨 데이터에 따른 코디 추천 텍스트 반영',
          detail: [
            '여러 API의 응답을 효율적으로 관리하기 위해 Promise.all을 사용해 날씨 데이터를 비동기적으로 동시에 호출함으로써 데이터 수집 속도를 단축할 수 있었습니다.',
            '조건별 텍스트 매칭을 통해 날씨 변화에 따라 코디 추천 메시지가 업데이트되도록 구현해 사용자에게 동적인 인터페이스를 제공했습니다.',
          ],
        },
        {
          contribution: '반응형 UI 지원',
          detail: [
            'react-responsive의 useMediaQuery 훅을 활용해 특정 화면 크기 조건을 확인하고, 조건부 렌더링을 통해 모바일 환경에 맞는 스타일을 적용함으로써 사용자 경험을 개선하였습니다.',
          ],
        },
      ],
    },
  ...
  collaboration: [
    ...
    },
    {
      ...
      detail: [
        'Swagger로 백엔드 API 구조를 사전에 파악해 요청/응답 형식을 명확히 이해하고소통 효율성을 높였습니다.',
      ],
    },
  ],
};
